...
input_anno_files = sys.argv[3:]
output_control_file = sys.argv[1]
pc_site_file = sys.argv[2]

# 阳控
df_pc = pd.read_table(pc_site_file, sep='\t', header=None, names=[
                       'Chrom', 'Start', 'Gene'], dtype=object)
df_pc['PC'] = df_pc['Gene'] + '|' + df_pc['Chrom'] + ':' + df_pc['Start'].astype(str)
df_pc.drop('Gene', axis=1, inplace=True)
...
for infile in input_anno_files:
    if 'POS' in infile:
        # 阳控不需要计算, 使用参考文件注明位置信息
        continue
    elif 'NTC' in infile:
        # 阴控
        df_ntc = pd.read_table(infile, sep='\t', usecols=['Chrom', 'Start'])
        df_ntc['NTC'] = 'NTC'
    else:
        # 检测样本
        sample_num += 1
        df_batch = pd.read_table(infile, sep='\t', usecols=['Chrom', 'Start'])
        for _, row in df_batch.iterrows():
            sample_site_dict[(row['Chrom'], str(row['Start']))] += 1
